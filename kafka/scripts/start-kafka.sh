...
if [[ ! -z "$ADVERTISED_LISTENERS" ]]; then
  ...
  sed -r -i "s@^#?advertised.listeners=.*@advertised.listeners=$ADVERTISED_LISTENERS@g" $KAFKA_HOME/config/server.properties
fi

if [[ ! -z "$INTER_BROKER_LISTENER_NAME" ]]; then
  echo "inter broker listener name: ${INTER_BROKER_LISTENER_NAME}"
  sed -r -i "s@^#?inter.broker.listener.name=.*@inter.broker.listener.name=$INTER_BROKER_LISTENER_NAME@g" $KAFKA_HOME/config/server.properties
fi

if [[ ! -z "$LISTENER_SECURITY_PROTOCOL_MAP" ]]; then
  echo "listener security protocol map: ${LISTENER_SECURITY_PROTOCOL_MAP}"
  sed -r -i "s@^#?listener.security.protocol.map=.*@listener.security.protocol.map=$LISTENER_SECURITY_PROTOCOL_MAP@g" $KAFKA_HOME/config/server.properties
fi
...
$KAFKA_HOME/bin/kafka-storage.sh format --standalone -t $uuid -c $KAFKA_HOME/config/server.properties
